...
- **[beta]** [llmtrim](https://github.com/fkiene/llmtrim) by [fkiene](https://github.com/fkiene) — Local proxy that sits in front of the OpenAI-compatible model Hermes calls and compresses each request before it is sent: the tool schemas Hermes resends every turn, conversation history, MCP tool output, and scraped pages. Quality-gated so it never increases your bill, with no Hermes code change and no Hermes-specific build. Also runs as an MCP server, CLI, Rust library, and WebAssembly npm package. MPL-2.0.
